...
def _legacy_pnmi(coocurrence: DataArray) -> float:
...
def test_pnmi_matches_legacy_on_well_defined_inputs(rows: list[list[int]]) -> None:
    ...
    cooc = make_coocurrence(matrix)
    assert pnmi(cooc) == pytest.approx(_legacy_pnmi(cooc), abs=1e-8)
...
def test_pnmi_invariant_under_permutation(rows: list[list[int]], data: st.DataObject) -> None:
    ...
    base = pnmi(make_coocurrence(matrix.tolist()))
    ...
    assert pnmi(make_coocurrence(permuted.tolist())) == pytest.approx(base, abs=1e-12)
...
def test_pnmi_is_one_when_units_are_phone_deterministic(assignments: list[tuple[int, int]]) -> None:
    ...
    assert pnmi(make_coocurrence(matrix.tolist())) == pytest.approx(1.0, abs=1e-9)
...
def test_pnmi_single_phone_is_defined_as_zero() -> None:
    ...
    assert pnmi(make_coocurrence([[5, 3], [0, 0]])) == 0.0


def test_pnmi_empty_matrix_is_defined_as_zero() -> None:
    assert pnmi(make_coocurrence([[0, 0], [0, 0]])) == 0.0
